...
sys_open(void)
{
  ...
  // Убираем флаг O_NOFOLLOW из режима открытия
  omode &= ~O_NOFOLLOW;
...
sys_readlink(void)
{
  ...
  buf[n] = '\0';
  if (copyout(myproc()->pagetable, addr, buf, strlen(buf)) < 0) {
    end_op();
    return -1;
  }
...
sys_lstat(void)
{
  ...
  begin_op();
  // Используем namei_symlink с флагом follow_symlinks=0, чтобы не разыменовывать ссылки
  if((ip = namei_symlink(path, 0)) == 0){
    end_op();
    return -1;
  }
  ilock(ip);
  
  // Получаем информацию об inode в локальную переменную
  stati(ip, &st);
  
  // Копируем структуру stat в пользовательское пространство
